...
class CentralityEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, g: dgl.DGLGraph) -> torch.Tensor:
        num_nodes = x.shape[0]
        in_degree = g.in_degrees().long()  
        in_degree = self.decrease_to_max_value(in_degree, self.max_in_degree - 1)  

        x = x + self.z_in[in_degree]  
...
class load_PROPDataSetDGL(torch.utils.data.Dataset):
...
def self_loop(g):
    """
        ...
        This function is called inside a function in PROPDataset class.
    """
    new_g = dgl.DGLGraph()
